...
@Table(
        ...
        uniqueConstraints = @UniqueConstraint(
                ...
                columnNames = {"reservation_id", "performance_seat_id"}
        ),
        indexes = @Index(
                name = "idx_v1_reservation_seats_performance_seat",
                columnList = "performance_seat_id"
        )
)
@Getter
...
public class ReservationSeat extends BaseAuditEntity {
    ...
    @JoinColumn(
            name = "reservation_id",
            nullable = false,
            foreignKey = @ForeignKey(name = "fk_v1_reservation_seats_reservation")
    )
    ...
    private SeatGradeSnapshot grade;
