...
@Entity
@Table(
        name = "v1_member_social_accounts",
        uniqueConstraints = {
                @UniqueConstraint(
                        name = "uk_v1_member_social_accounts_provider_user",
                        columnNames = {"provider", "provider_user_id"}
                ),
                @UniqueConstraint(
                        name = "uk_v1_member_social_accounts_member_provider",
...
public class MemberSocialAccount extends BaseAuditEntity {
    ...
    @JoinColumn(
            name = "member_id",
            nullable = false,
            foreignKey = @ForeignKey(name = "fk_v1_member_social_accounts_member")
    )
