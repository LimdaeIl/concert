...
export async function getPerformances(
  ...
  const { data } =
      await publicApiClient.get<PerformanceListResponse>(
...
export async function getPerformance(
  ...
  const { data } =
      await publicApiClient.get<PerformanceDetailResponse>(
...
export async function getPerformanceSeats(
    performanceId: number,
): Promise<PerformanceSeatListResponse> {
  const { data } =
      await publicApiClient.get<PerformanceSeatListResponse>(
